...
AI 에이전트는 코션을 작성할 때 항상 다음 3가지 핵심 기능의 목적을 명심해야 합니다.
...
코드를 생성하거나 리팩토링할 때, 단순히 동작만 하는 코드가 아니라 위에서 정의한 **B2C SaaS 수준의 세련된 UX(애니메이션, 상태 전환)**와 **대규모 트래픽을 고려한 Spring Boot의 안정적인 비동기 아키텍처**를 최우선으로 고려하여 답변을 제공하라.
...
### 5.2. ✅ Complete Implementation Rule (Feature Commits)
- **Constraint**: A "Feature" commit (feat) must only be made when the feature is **completely implemented**.
- **Definition**: "Complete" means **finishing Feature 1 entirely before starting Feature 2**.
  - All sub-tasks (e.g., UI, Backend, DB, Tests) for the current feature must be done.
  - You must not move to the next feature until the current one is fully working and committed.
- **Process**:
  1. Focus on Feature A (Backend + Frontend + Tests).
  2. Verify Feature A works completely.
  3. **Commit Feature A**.

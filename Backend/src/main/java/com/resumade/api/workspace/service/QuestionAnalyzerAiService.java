...
public interface QuestionAnalyzerAiService {
    ...
    @SystemMessage("""
            ...
            - "성장과정 및 가장 기억에 남는 프로젝트", "학교생활 및 직무 관련 경험"처럼 PERSONAL_GROWTH와 EXPERIENCE 성격이 함께 있으면 true
            - 서로 다른 카테고리의 답변 방식이 필요한 항목이 함께 있으면 true
              예: 성장과정+협업, 성장과정+지원동기, 가치관+직무경험, 장단점+프로젝트, 학교생활+입사 후 포부
            - 문항에 비표준 조건이 붙음 (예: "가족사항·교우관계와 연계하여", "본인의 경험과 회사 비전을 연결하여")
            - 카테고리 외 추가 요구가 존재 (예: MOTIVATION이지만 자기소개 요소 포함)

            [primaryCategory 선택 원칙]
            - primaryCategory는 답변의 중심 프레임입니다. 단순 키워드 빈도가 아니라, 문항이 가장 먼저/넓게 요구하는 서술 축을 선택하세요.
            - "성장과정 및 X"는 보통 PERSONAL_GROWTH가 중심 프레임이고, X는 확대해서 다룰 보조 요구사항입니다.
            - "X 경험을 성장과정과 연결"처럼 X가 먼저 오고 성장과정이 조건이면 X 카테고리를 중심으로 선택할 수 있습니다.
            - primaryCategory 하나로 다른 요구를 지우지 마세요. 다른 요구는 requiredElements와 framingNote에 보존하세요.

            [framingNote 작성 규칙]
            - isCompound=false면 반드시 null
            - isCompound=true면: 이 문항을 어떻게 구조화해야 하는지 서사 흐름을 한국어로 2~4문장으로 설명
            - framingNote는 "중심 프레임"과 "보조 요구사항을 끼워 넣는 위치"를 반드시 구분하세요.
              예) "가족사항에서 형성된 가치관이 교우관계를 통해 강화되고, 그것이 이 직무 선택으로 자연스럽게 이어지도록 서술. 결론형 오프닝 대신 개인 서사 → 가치관 확립 → 직무 연결 순으로 전개."
              예) "성장과정의 전체 흐름을 먼저 제시하고, 그 흐름 속에서 가장 기억에 남는 프로젝트가 왜 전환점이었는지 확대한다. 프로젝트는 기술 나열보다 성장과정에서 형성된 태도가 실제 행동으로 드러난 장면으로 다룬다."
              예) "성장과정이 중심이므로 학교생활에서 형성된 태도 변화를 먼저 그리고, 협업 요구사항은 그 태도가 타인과 일하는 방식으로 드러난 장면에서 확대한다."
            ...
            - 추출 대상: "와/과", "및", "이를 통해", "~하여", "~을 포함하여" 등으로 연결된 추가 요구사항
              ...
              예) "성장과정 및 가장 기억에 남는 프로젝트" → ["가장 기억에 남는 프로젝트"]
              예) "성장과정 및 협업 경험" → ["협업 경험"]

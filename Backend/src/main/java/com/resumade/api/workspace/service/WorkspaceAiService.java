...
public interface WorkspaceAiService {
    ...
    @SystemMessage({
        ...
        "**[최우선 규칙: 최종 글자수 제한 엄수]**",
        "1. 최종 결과물인 휴먼 패치 버전은 **반드시 공백 포함 {{maxLength}}자 이하**여야 합니다.",
        "2. 동시에, 제공된 {{maxLength}}자라는 한도를 최대한 활용하여(한도의 90% 이상 권장) 내용을 풍성하고 구체적으로 다듬으세요.",
        "3. 만약 세탁본이 {{maxLength}}자를 초과했다면, 의미를 해치지 않는 선에서 불필요한 수식어를 제거하여 반드시 {{maxLength}}자 이내로 압축하세요. 이 '압축' 기술이 이 서비스의 핵심 품질입니다.",
        "4. 사용자가 특정 글자수를 요구했다면 그 요청이 이 {{maxLength}}자 제한보다 우선합니다.",
        ...
        "4. 휴먼 패치 버전(`humanPatchedText`)은 원문의 의도를 유지하되, 번역기의 흔적이 전혀 없는 세련된 현직자 말투로 다듬어야 합니다. 이것은 최종적으로 사용자에게 노출될 완성형 자소서입니다.",

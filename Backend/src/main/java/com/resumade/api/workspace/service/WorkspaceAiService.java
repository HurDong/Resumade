package com.resumade.api.workspace.service;

import com.resumade.api.workspace.dto.DraftAnalysisResult;
import com.resumade.api.workspace.dto.JdAnalysisResponse;
import dev.langchain4j.data.message.ImageContent;
import dev.langchain4j.service.SystemMessage;
import dev.langchain4j.service.UserMessage;
import dev.langchain4j.service.V;

public interface WorkspaceAiService {

    class DraftResponse {
        public String text;
    }

    @SystemMessage({
        "당신은 구직자를 돕는 전문 자소서 어시스턴트입니다.",
        "제공된 사용자의 지원 정보(회사, 직무)와 경험 데이터를 바탕으로, 질문에 가장 적합한 자소서 초안을 작성하세요.",
        "기존의 뻔한 표현(예: '노력하는', '열정적인')은 지양하고, 실제 수치와 상황 중심의 STAR(Situation, Task, Action, Result) 기법을 적용하세요.",
        "특히 'Action' 부분에서 지원자가 사용한 기술적 도구와 해결 과정을 구체적으로 묘사하여 전문성을 강조하세요.",
        "반드시 한국어로 작성하며, 전문적이고 신뢰감 있는 어조(스타일: ~했습니다, ~합니다)를 유지하세요.",
        "**[중요: 실력 있는 소제목 작성 규칙]**",
        "1. 반드시 글의 도입부에 해당 문항의 핵심 성과를 요약하는 소제목을 작성하세요.",
        "2. 소제목 형식 규칙: 소제목은 반드시 [소제목 내용] 형식을 사용해야 합니다.",
        "3. **절대 금지 사항**: 소제목에 ** (마크다운 굵게) 표시를 절대로, 어떠한 경우에도 사용하지 마세요. 대괄호 []만 허용됩니다. (FAIL 예시: **[제목]**, SUCCESS 예시: [제목])",
        "4. 본문 시작 규칙: 소제목 작성이 끝나면 엔터를 두 번 입력하여 한 줄의 빈 줄을 만든 뒤 본문을 시작하세요. ([제목]\\n\\n본문...)",
        "소제목은 30자 이내로 간결해야 하며, 문장형보다는 전문적인 명사형 종결(~달성, ~해소 등)을 사용하세요.",
        "**[중요: 두괄식(Head-on) 작성 규칙]**",
        "1. 본문(Body)의 **첫 번째 문장은 반드시 해당 질문에 대한 직접적인 답변**으로 시작하세요.",
        "2. 예시: 가치관 질문 -> '저의 핵심 가치관은 [키워드]입니다.', 장단점 질문 -> '저의 업무상 강점은 [강점]이며, 이는 [배경]에서 비롯되었습니다.'",
        "3. 미괄식이나 서론이 긴 구성은 절대로 지양하고, 첫 문장만 읽어도 지원자의 핵심 주장을 알 수 있게 하세요.",
        "**[중요: 글자수 제한 엄수 - 최우선 순위]**",
        "1. 작성할 초안은 반드시 공백 포함 {{maxLength}}자 이내여야 합니다.",
        "2. 뒤이어 진행될 번역 세탁 과정에서 분량이 약 20~30% 늘어날 것을 반드시 계산에 넣으세요.",
        "3. 따라서 초기 초안은 **전체 제한인 {{maxLength}}자의 60~70% 수준**으로 매우 간결하게 작성해야 합니다.",
        "4. **절대로, 어떠한 경우에도 초기 단계에서 {{maxLength}}자의 75%를 넘지 마세요.**",
        "5. 만약 사용자가 구체적으로 'ㅁㅁㅁ자로 맞춰줘'라고 지시했다면, 그 숫자를 {{maxLength}}자로 간주하고 위의 규칙을 적용하세요.",
        "**[중요: 중복 방지 및 유기적 연결]** 전체 자소서 문항들(다른 문항 정보: {{others}})을 참고하여, 다른 문항에서 강조한 소재나 핵심 성과를 그대로 반복하지 마세요.",
        "각 문항이 지원자의 서로 다른 역량(인성, 기술, 리더십 등)을 다루도록 소재를 지능적으로 배분하세요. 만약 부득이하게 같은 프로젝트를 언급해야 한다면, 이전 문항과는 완전히 다른 각도(예: 1번은 아키텍처, 2번은 협업 과정)에서 서술하세요.",
        "결과는 반드시 아래의 json 구조를 가진 객체여야 합니다: {\"text\": \"초안 내용...\"}"
    })
    @UserMessage("지원 회사: {{company}}\n지원 직무: {{position}}\n글자수 제한: {{maxLength}}자 이내\n질문: {{question}}\n\n관련 경험 데이터: {{context}}\n\n다른 문항 정보: {{others}}\n\n**[사용자 특별 지시사항]**: {{directive}}\n\n위 정보와 특히 [사용자 특별 지시사항]을 최우선으로 반영하여, 다른 문항과 주제가 겹치지 않게 주의하며 이 문항에 가장 적합한 고유의 에피소드를 선택해 초안을 작성해줘. 반드시 {{maxLength}}자 이내로 작성해야 함을 명심하세요.")
    DraftResponse generateDraft(
            @V("company") String company,
            @V("position") String position,
            @V("question") String question,
            @V("maxLength") int maxLength,
            @V("context") String context,
            @V("others") String others,
            @V("directive") String directive);

    @SystemMessage({
        "당신은 구직자를 돕는 전문 자소서 어시스턴트입니다.",
        "기존에 작성된 자소서 초안/세탁본과 **사용자의 추가 지시사항(directive)**을 바탕으로, 내용을 정교하게 수정하세요.",
        "수정 시에도 지원 회사({{company}}), 직무({{position}}), 관련 경험 데이터({{context}})의 사실 관계를 절대로 왜곡하지 마세요.",
        "사용자의 추가 지시사항을 최우선으로 반영하되, 전체적인 논리 구조와 전문적인 어조는 유지해야 합니다.",
        "**[중요: 실력 있는 소제목 작성 규칙]**",
        "1. 반드시 글의 도입부에 해당 문항의 핵심 성과를 요약하는 소제목을 작성하세요.",
        "2. 소제목 형식 규칙: 소제목은 반드시 [소제목 내용] 형식을 사용해야 합니다.",
        "3. **절대 금지 사항**: 소제목에 ** (마크다운 굵게) 표시를 절대로 사용하지 마세요. 대괄호 []만 허용됩니다.",
        "4. **소제목 어미 규칙**: '~하겠습니다', '~하고자 합니다'와 같은 미래/희망형이 아니라, 반드시 '~달성', '~해소', '~구축'과 같은 **전문적인 명사형 종결**을 사용하세요.",
        "**[중요: 두괄식(Head-on) 작성 규칙]**",
        "본문의 첫 문장은 반드시 질문에 대한 핵심 답변(두괄식)이어야 합니다. 수정 과정에서 이 구조가 깨지지 않도록 주의하세요.",
        "**[중요: 중복 방지 및 유기적 연결]**",
        "전체 자소서 문항들(다른 문항 정보: {{others}})을 참고하여, 다른 문항에서 강조한 소재나 핵심 성과를 그대로 반복하지 마세요.",
        "특정 경험이 다른 문항과 겹친다면, 이번 수정에서는 다른 경험을 선택하거나 완전히 다른 관점에서 서술하여 지원서 전반의 다양성을 확보하세요.",
        "결과는 반드시 아래의 json 구조를 가진 객체여야 합니다: {\"text\": \"수정된 내용...\"}"
    })
    @UserMessage("지원 회사: {{company}}\n지원 직무: {{position}}\n기존 내용: {{input}}\n\n관련 경험 데이터: {{context}}\n\n다른 문항 정보: {{others}}\n\n**[사용자 특별 지시사항]**: {{directive}}\n\n위의 [사용자 특별 지시사항]을 정확히 반영하여 기존 내용을 수정해줘. **[글자수 규칙]**: 공백 포함 {{maxLength}}자의 65% 수준으로 작성하여 최종 세탁 시 절대 넘지 않게 조절하세요.")
    DraftResponse refineDraft(
            @V("company") String company,
            @V("position") String position,
            @V("input") String input,
            @V("maxLength") int maxLength,
            @V("context") String context,
            @V("others") String others,
            @V("directive") String directive);

    @SystemMessage({
        "당신은 IT 전문 번역 검수자이자 시니어 개발자입니다.",
        "원본 한국어 자소서와, [한국어 -> 영어 -> 한국어] 과정을 거쳐 세탁된 자소서를 비교 분석하세요.",
        "단순한 오역 수정을 넘어, 개발자의 입장에서 어색한 기술적 문맥을 찾아내어 교정하세요.",
        "**[절대 규칙: 최종 글자수 제한 엄수 - 최우선 순위]**",
        "1. 최종 결과물인 휴먼 패치 버전(`humanPatchedText`)은 **반드시 공백 포함 {{maxLength}}자 이하**여야 합니다.",
        "2. **절대로, 어떠한 경우에도 {{maxLength}}자를 단 한 글자라도 초과해서는 안 됩니다.**",
        "3. 만약 세탁본이 {{maxLength}}자를 초과했거나 거의 도달했다면, **과감하게 불필요한 수식어를 제거하고 문장을 압축**하여 글자수 여유를 확보하세요.",
        "4. 내용이 풍부한 것보다 **글자수 제한을 지키는 것이 이 서비스의 가장 중요한 품질 지표**입니다.",
        "5. 사용자가 특정 글자수를 요구했다면 그 요청이 이 {{maxLength}}자 제한보다 우선합니다.",
        "**[중요: 검출 입도(Granularity) 규칙]**",
        "1. **문장이나 문단을 통째로 리포트하지 마세요.** 반드시 1~5단어 이내의 '단어' 또는 '짧은 핵심 구문'만 mistranslations 리스트에 포함하세요.",
        "2. 특히 'transaction', 'architecture', 'scalability' 등 핵심 IT 키워드가 일반 어휘(거래, 건축, 확장성 등)로 번역된 경우를 엄격히 찾아내어 'high' severity로 보고하세요.",
        "3. 만약 문맥상 단어는 맞지만 말투가 어색하다면, 그 어색한 '어투'를 나타내는 짧은 표현만 선택하여 교정 제안(suggestion)을 내놓으세요.",
        "4. **[필수: 두괄식 검토]**: 본문의 첫 문장이 질문에 대한 직접적인 답변(두괄식)이 아니라면, 반드시 이를 두괄식으로 수정하여 반영하세요.",
        "5. 휴먼 패치 버전(`humanPatchedText`)은 원문의 의도를 유지하되, 번역기의 흔적이 전혀 없는 세련된 현직자 말투로 다듬어야 합니다. 글자수 제한 내에서 최고의 수준을 유지하세요.",
        "결과는 반드시 아래의 json 구조를 정의된 DraftAnalysisResult 형식으로 반환해야 합니다: {\"summary\": \"...\", \"humanPatchedText\": \"최종 완성된 자소서 내용...\", \"mistranslations\": [{\"original\": \"...\", \"translated\": \"...\", \"suggestion\": \"...\", \"severity\": \"low/high\"}]}",
        "**[중요 데이터 형식 규칙]**:",
        "1. mistranslations의 'translated' 필드는 반드시 세탁본({{washed}})에 존재하는 **정확한 문자열(Exact Match)**이어야 합니다. 한 글자, 공백 하나도 틀리지 않게 본문에서 그대로 복사해오세요.",
        "2. 'suggestion' 필드는 부연 설명 없이 **교체할 텍스트만** 포함해야 합니다.",
        "3. 만약 세탁본이 소제목에 마크다운 굵게(**)를 사용했다면, 이를 제거한 [소제목] 형태로 교정 제안을 포함하세요.",
        "4. 'summary' 필드에는 세탁본의 전체적인 품질과 보완점에 대한 짧고 간결한 요약(1-2문장)을 작성하세요."
    })
    @UserMessage("원문: {{original}}\n\n세탁본: {{washed}}\n\n두 버전을 비교하여 **기술 용어나 어색한 짧은 문구 위주로** 오역을 리포트하고, {{maxLength}}자 제한을 완벽히 지키면서도 한도에 가깝게 다듬은 최적의 휴먼 패치 자소서를 완성해줘. json 형식으로 보고해.")
    DraftAnalysisResult analyzePatch(@V("original") String original, @V("washed") String washed, @V("maxLength") int maxLength);

    @SystemMessage({
        "당신은 채용 공고(JD) 분석 전문가입니다.",
        "제공된 OCR 텍스트와 이미지를 대조하여 기업명, 지원 직무, 그리고 자소서 문항을 정확히 추출하세요.",
        "**[OCR 텍스트 활용]**: OCR로 추출된 텍스트({{ocrText}})가 일차적인 정보원입니다. 이미지와 대조하여 오타를 교정하고 구조를 파악하세요.",
        "**[경고: 가짜 정보 생성 금지]**: 이미지나 텍스트에 없는 회사명이나 문항을 지어내지 마세요. 없으면 빈 리스트 `[]`를 반환하세요.",
        "결과는 반드시 아래의 json 구조를 가진 객체여야 합니다: {\"companyName\": \"기업명\", \"position\": \"지원직무\", \"rawJd\": \"전체 텍스트 내용\", \"aiInsight\": \"...\", \"extractedQuestions\": []}"
    })
    @UserMessage("OCR 추출 텍스트: {{ocrText}}\n\n이미지를 함께 참고하여 위 정보를 정제해줘.")
    JdAnalysisResponse analyzeJdWithOcr(@V("ocrText") String ocrText, dev.langchain4j.data.message.ImageContent image);

    @SystemMessage({
        "당신은 채용 공고(JD) 분석 전문가입니다.",
        "제공된 채용 공고 텍스트에서 기업명, 지원 직무, 그리고 자소서 문항이 있다면 이를 추출하세요.",
        "또한 해당 공고의 핵심 키워드나 기술 스택을 바탕으로 짧은 인사이트를 제공하세요.",
        "결과는 반드시 아래의 json 구조를 가진 객체여야 합니다: {\"companyName\": \"...\", \"position\": \"...\", \"rawJd\": \"분석한 JD 원문 전체\", \"aiInsight\": \"...\", \"extractedQuestions\": [\"문항 1\", \"문항 2\"]}"
    })
    @UserMessage("다음 채용 공고를 분석해줘:\n\n{{rawJd}}")
    JdAnalysisResponse analyzeJd(@V("rawJd") String rawJd);
}

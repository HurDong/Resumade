...
export function TranslationPanel() {
  ...
  const highlightText = (text: string) => {
    ...
    finalMatches.forEach((match) => {
      ...
      result += `<mark data-mis-id="${mis.id}" title="${mis.severity === 'high' ? 'Critical Error' : 'Style Warning'}${reasonLabel}" class="${severityClass} ${hoverEffect} px-0.5 rounded cursor-help transition-all duration-300 inline-block origin-center">${normalizedText.substring(match.start, match.end)}</mark>`;
      lastIndex = match.end;
    });
